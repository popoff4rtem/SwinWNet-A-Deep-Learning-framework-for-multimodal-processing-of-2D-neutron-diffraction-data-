...
class ScaleAwarePatchEmbed(nn.Module):
    def __init__(
        self,
        patch_size=2,
        in_chans=1,
    ...
    def forward(self, x, scale_factor=1):
        """
        scale_factor = 1 → LR (250×480)
        scale_factor = 2 → HR (500×960)
        """
        B, C, H, W = x.shape

        if scale_factor > 1:
            # 🔑 меняем dilation и stride, НЕ веса
            self.proj.dilation = (scale_factor, scale_factor)
            self.proj.stride = (self.patch_size * scale_factor,
                                self.patch_size * scale_factor)
        else:
            self.proj.dilation = (1, 1)
            self.proj.stride = (self.patch_size, self.patch_size)

        # Padding if necessary to make H and W divisible by patch_size
        pad_h = (self.patch_size * scale_factor  - H % self.patch_size * scale_factor) % self.patch_size * scale_factor
        pad_w = (self.patch_size * scale_factor - W % self.patch_size * scale_factor) % self.patch_size * scale_factor
...
class SwinEncoder(nn.Module):
    ...
    def forward(self, x, resolution):
        ...
        return skips, res_skips, resolution
...
class SegmentationHead(nn.Module):
    ...
    def forward(self, x, resolution, scale_factor=1):
        """
        x: [B, N, C]
        ...
        H_patch = H // (self.patch_size*scale_factor)
        W_patch = W // (self.patch_size*scale_factor)
        ...
        x = nn.functional.interpolate(x, scale_factor=(self.patch_size*scale_factor), mode='bilinear')
...
class SwinUNet(nn.Module):
    def __init__(self, patch_size=2, in_chans=1, embed_dim=48, depths=[2, 2, 6, 2], num_heads=[3, 6, 12, 24],
        ...
        self.patch_embed = ScaleAwarePatchEmbed(
                    patch_size=patch_size,
                    in_chans=in_chans,
                    embed_dim=embed_dim,
                    )

        self.encoder = SwinEncoder(embed_dim=embed_dim, 
                    depths=depths, 
                    num_heads=num_heads, 
                    window_size=window_size,
                    mlp_ratio=mlp_ratio, 
    ...
    def forward(self, x):
        # embed
        x_patch, padded_res = self.patch_embed(x, scale_factor=1)

        resolution = (padded_res[0] // self.patch_embed.patch_size, padded_res[1] // self.patch_embed.patch_size)

        # encode
        skips, skip_res_list, bott_res = self.encoder(x_patch, resolution)
        ...
        # segmentation map
        seg = self.head(x_dec, padded_res)

        return seg

class UpscalingHead(nn.Module):
    """
    Swin-based super-resolution head.
    Input  : [B, N, embed_dim]  (H_patch × W_patch)
    Output : [B, 1, 2H, 2W]
    """
    def __init__(
        self,
        ...
        for i in range(2):
            ...
            self.ups.append(PatchExpanding(dim=embed_dim))

            self.swin_blocks.append(
                # --- refinement in HR token space ---
                BasicLayer(dim=embed_dim // 2,
                    depth=depth,
                    num_heads=num_heads,
                    window_size=window_size,
                    mlp_ratio=mlp_ratio,
            ...
            embed_dim = embed_dim // 2  # prepare for next iteration
    ...
    def forward(self, x, resolution):
        ...
        for i in range(2):
            ...
            # swin processing
            x = self.swin_blocks[i](x, new_res)

            resolution = new_res

        H_hr, W_hr = resolution

        # --- tokens → image ---
        x = x.transpose(1, 2).reshape(B, C // 4, H_hr, W_hr)
...
class SwinUNetSR(nn.Module):
    def __init__(self, patch_size=2, in_chans=1, embed_dim=48, depths=[2, 2, 6, 2], num_heads=[3, 6, 12, 24],
        ...
        self.head = UpscalingHead(embed_dim=embed_dim,
                    patch_size=patch_size,
                    window_size=window_size,
                    num_heads=3,
                    depth=2,
    ...
    def forward(self, x):
        ...
        # segmentation map
        upscaled = self.head(x_dec, padded_res)

        return upscaled[:, :, :result_H, :result_W]
...
class SwinWNet(nn.Module):
    def __init__(self, patch_size=2, in_chans=1, error_matrix=False, embed_dim=48, depths=[2, 2, 6, 2], num_heads=[3, 6, 12, 24],
                 window_size=5, mlp_ratio=4., qkv_bias=True, drop=0., attn_drop=0., drop_path=0.):
        super().__init__()
        ...
        self.segmentator_encoder = SwinEncoder(embed_dim=embed_dim, 
                    depths=depths, 
                    num_heads=num_heads, 
                    window_size=window_size,
                    mlp_ratio=mlp_ratio, 
        ...
        self.segmentator_bottleneck = Bottleneck(dim=embed_dim*8, 
                    num_heads=num_heads[-1], 
                    window_size=window_size)
        
        self.segmentator_decoder = SwinDecoder(embed_dim=embed_dim, 
                    depths=depths, 
                    num_heads=num_heads,
                    window_size=window_size, 
                    mlp_ratio=mlp_ratio, 
        ...
        self.segmentator_head = SegmentationHead(embed_dim=embed_dim, 
            patch_size=patch_size)
        
        self.ca_seg_to_sr = MultiScaleCrossAttention(dims=[embed_dim*4, embed_dim*8], 
                                                    heads=[3, 3])
        

        self.ca_sr_to_seg = MultiScaleCrossAttention(dims=[embed_dim*4, embed_dim*8], 
                                            heads=[3, 3])
        
        self.upscaler_encoder = SwinEncoder(embed_dim=embed_dim, 
                    depths=depths, 
                    num_heads=num_heads, 
                    window_size=window_size,
                    mlp_ratio=mlp_ratio, 
        ...
        self.upscaler_bottleneck = Bottleneck(dim=embed_dim*8, 
                    num_heads=num_heads[-1], 
                    window_size=window_size)
        
        self.upscaler_decoder = SwinDecoder(embed_dim=embed_dim, 
                    depths=depths, 
                    num_heads=num_heads,
                    window_size=window_size, 
                    mlp_ratio=mlp_ratio, 
        ...
        self.upscaler_head = UpscalingHead(error_matrix=error_matrix,
                    embed_dim=embed_dim,
                    patch_size=patch_size,
                    window_size=window_size,
                    num_heads=3,
    ...
    def segment_1(self, x):
        ...
        # encode
        skips_segmentator, skip_res_list, bott_res = self.segmentator_encoder(x_patch, resolution)

        # bottleneck
        x_bottleneck = self.segmentator_bottleneck(skips_segmentator[-1], bott_res)

        # decode
        x_dec, dec_res = self.segmentator_decoder(x_bottleneck, bott_res, skips_segmentator, skip_res_list)

        # segmentation map
        seg = self.segmentator_head(x_dec, padded_res)

        return seg, skips_segmentator
        
    def upscale(self, x, skips_segmentator):
        ...
        # encode
        skips_upscaler, skip_res_list, bott_res = self.upscaler_encoder(x_patch, resolution)

        ca_skips = self.ca_seg_to_sr([skips_upscaler[-2], skips_upscaler[-1]], [skips_segmentator[-2], skips_segmentator[-1]]) # target_skips, source_skips

        skips_upscaler[-2], skips_upscaler[-1] = ca_skips

        # bottleneck
        x_bottleneck = self.upscaler_bottleneck(skips_upscaler[-1], bott_res)

        # decode
        x_dec, dec_res = self.upscaler_decoder(x_bottleneck, bott_res, skips_upscaler, skip_res_list)

        # segmentation map
        upscaled = self.upscaler_head(x_dec, padded_res)

        return upscaled[:, :, :result_H, :result_W], skips_upscaler
        
    def segment_2(self, x, skips_upscaler):

        # embed
        x_patch, padded_res = self.patch_embed(x, scale_factor=2)

        resolution = (padded_res[0] // (2*self.patch_embed.patch_size), padded_res[1] // (2*self.patch_embed.patch_size))

        # encode
        skips_segmentator, skip_res_list, bott_res = self.segmentator_encoder(x_patch, resolution)

        ca_skips = self.ca_sr_to_seg([skips_segmentator[-2], skips_segmentator[-1]], [skips_upscaler[-2], skips_upscaler[-1]]) # target_skips, source_skips

        skips_segmentator[-2], skips_segmentator[-1] = ca_skips
        ...
        # segmentation map
        seg = self.segmentator_head(x_dec, padded_res, scale_factor=2)
